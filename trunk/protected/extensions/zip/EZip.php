...
class EZip extends CApplicationComponent {
    ...
    public function makeZip ($src, $dest, $flag = ZIPARCHIVE::OVERWRITE)
    {
        $zip = new ZipArchive;
        $src = is_array($src) ? $src : array($src);
        if ($zip->open($dest, $flag) === true)
        {
            foreach ($src as $item)
                if (file_exists($item))
                    $this->addZipItem($zip, realpath(dirname($item)).DIRECTORY_SEPARATOR, realpath($item).DIRECTORY_SEPARATOR);
            $zip->close();
            return true;
        }
        return false;
