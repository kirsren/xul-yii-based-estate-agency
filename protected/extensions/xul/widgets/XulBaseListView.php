<?php
/**
 * XulBaseListView class file.
 *
 */
abstract class XulBaseListView extends CWidget
{
	/**
	 ...
	 */
	public $pager=array('class'=>'XulButtonPager');
	/**
	...
	public function run()
	{
		...
		echo Xul::xopenTag($this->tagName,$this->xulOptions)."\n";

		$this->renderContent();
		$this->renderKeys();

		echo Xul::xcloseTag($this->tagName);
	}
	...
	{
		$emptyText=$this->emptyText===null ? Yii::t('xul','No results found.') : $this->emptyText;
		echo Xul::xtag('description', array('class'=>'empty'), $emptyText);
	}
	...
	{
		echo Xul::xopenTag('box',array(
			'class'=>'keys',
			'style'=>'display:none',
			'title'=>Yii::app()->getRequest()->getUrl(),
		));
		foreach($this->dataProvider->getKeys() as $key)
			echo Xul::xtag('box',array(), Xul::encode($key));
	...
	public function renderSummary()
	{
		...
		echo Xul::xopenTag('box', array('class'=> $this->summaryCssClass));
		if($this->enablePagination)
		{
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Yii::t('xul','Displaying {start}-{end} of {count} result(s).');
		...
		{
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Yii::t('xul','Total {count} result(s).');
			echo strtr($summaryText,array(
				'{count}'=>$count,
				'{start}'=>1,
				'{end}'=>$count,
	...
	public function renderPager()
	{
        ...
        if(substr($class, 0,3) == 'Xul'){
            Yii::import('ext.xul.widgets.pagers.' . $class);
        }
		...
		{
			echo Xul::xtag('hbox',array('class'=>$this->pagerCssClass),$this->widget($class,$pager, true));
		}
		else
