...
class Xul extends CHtml
{
   	...
   	public static function xtag($tag,$xulOptions=array(),$content=false,$closeTag=true)
	{
	   if(self::$namespace){ // with namespace
            ...
            if($content===false)
    			...
    			return $closeTag ? $html.'>'.$content.'</'. self::$namespace . ':' .$tag.'>' : $html.'>'.$content;
    ...
    public static function defaultXULHead($ns = 'x', $defaultSkin=true){
        ...
        if($defaultSkin) $head .= '<?xml-stylesheet href="chrome://global/skin" type="text/css"?>'."\n";
	 ...
	 */
	public static function statefulForm($action='',$method='post',$xulOptions=array())
	{
		return self::form($action,$method,$xulOptions)."\n".
			self::xtag('description',array('style'=>'display:none'),self::pageStateField(''));
	}
	...
	{
		return self::xtag('textbox', array('style'=>'display:none', 'id'=>CController::STATE_INPUT_NAME, 'value'=>$value));
	}
	 ...
	 * @see clientChange
	 */
	public static function button($label='button',$xulOptions=array())
	{
	...
	/**
	 * Generates a button using XUL button tag.
	 ...
	 * @since 1.0.8
	 */
	public static function XULButton($label='button',$xulOptions=array())
	{
		if(!isset($xulOptions['name']))
			$xulOptions['name']=self::ID_PREFIX.self::$count++;
		if(!isset($xulOptions['type']))
			$xulOptions['type']='button';
		self::clientChange('click',$xulOptions);
		return self::tag('button',$xulOptions,$label);
	}
     ...
     */
    public static function labelAndDescription($label, $value, $separator='', $xulOptions=array(), $id=false){
	...
	public static function textField($name,$value='',$xulOptions=array())
	{
		...
		return self::inputField('text',$name,$value,$xulOptions);
	}
	 ...
	 * @see inputField
	 */
	public static function passwordField($name,$value='',$xulOptions=array())
	{
		self::clientChange('change',$xulOptions);
		return self::inputField('password',$name,$value,$xulOptions);
	}
	...
	public static function textArea($name,$value='',$xulOptions=array())
	{
		...
		else if($xulOptions['id']===false)
			unset($xulOptions['id']);
		self::clientChange('change',$xulOptions);
		return self::tag('textarea',$xulOptions,isset($xulOptions['encode']) && !$xulOptions['encode'] ? $value : self::encode($value));
	}
	 ...
	 * @see listData
	 */
	public static function dropDownList($name,$select,$data,$xulOptions=array())
	{
		$xulOptions['name']=$name;
		if(!isset($xulOptions['id']))
			$xulOptions['id']=self::getIdByName($name);
			...
			unset($xulOptions['id']);
		self::clientChange('change',$xulOptions);
		$options="\n".self::listOptions($select,$data,$xulOptions);
	...
	{
		$template=isset($xulOptions['template'])?$xulOptions['template']:'{input} {label}';
		...
		$labelOptions=isset($xulOptions['labelOptions'])?$xulOptions['labelOptions']:array();
...
EOD;
	 ...
	 */
	public static function radioButtonList($name,$select,$data,$xulOptions=array())
	{
		$template=isset($xulOptions['template'])?$xulOptions['template']:'{input} {label}';
		$separator=isset($xulOptions['separator'])?$xulOptions['separator']:"<br/>\n";
		unset($xulOptions['template'],$xulOptions['separator']);
	 ...
	 * @see ajax
	 */
	public static function ajaxLink($text,$url,$ajaxOptions=array(),$xulOptions=array())
	{
	 ...
	 */
	public static function ajaxButton($label,$url,$ajaxOptions=array(),$xulOptions=array())
	{
		$ajaxOptions['url']=$url;
		$xulOptions['ajax']=$ajaxOptions;
		return self::button($label,$xulOptions);
	 ...
	 * @return string the generated button
	 */
	public static function ajaxSubmitButton($label,$url,$ajaxOptions=array(),$xulOptions=array())
	{
	...
	public static function ajax($options)
	{
		...
		if(isset($options['update']))
		{
			if(!isset($options['success']))
				$options['success']='js:function(XUL){jQuery("'.$options['update'].'").XUL(XUL)}';
			unset($options['update']);
		}
		if(isset($options['replace']))
		{
			if(!isset($options['success']))
				$options['success']='js:function(XUL){jQuery("'.$options['replace'].'").replaceWith(XUL)}';
	 ...
	 * @see activeInputField
	 */
	public static function activePasswordField($model,$attribute,$xulOptions=array())
	{
		self::resolveNameID($model,$attribute,$xulOptions);
		self::clientChange('change',$xulOptions);
	...
	public static function activeTextArea($model,$attribute,$xulOptions=array())
	{
		...
		return self::tag('textarea',$xulOptions,isset($xulOptions['encode']) && !$xulOptions['encode'] ? $text : self::encode($text));
	}
	...
	public static function activeFileField($model,$attribute,$xulOptions=array())
	{
		...
		$hiddenOptions=isset($xulOptions['id']) ? array('id'=>self::ID_PREFIX.$xulOptions['id']) : array('id'=>false);
		return self::hiddenField($xulOptions['name'],'',$hiddenOptions)
			. self::activeInputField('file',$model,$attribute,$xulOptions);
	}
	...
	public static function activeRadioButton($model,$attribute,$xulOptions=array())
	{
		...
		if(!isset($xulOptions['value']))
			$xulOptions['value']=1;
		if(!isset($xulOptions['checked']) && self::resolveValue($model,$attribute)==$xulOptions['value'])
			$xulOptions['checked']='checked';
		self::clientChange('click',$xulOptions);
	 ...
	 * @see listData
	 */
	public static function activeDropDownList($model,$attribute,$data,$xulOptions=array())
	{
		self::resolveNameID($model,$attribute,$xulOptions);
	 ...
	 * @see listData
	 */
	public static function activeListBox($model,$attribute,$data,$xulOptions=array())
	{
		if(!isset($xulOptions['size']))
			$xulOptions['size']=4;
	 ...
	 * @see checkBoxList
	 */
	public static function activeCheckBoxList($model,$attribute,$data,$xulOptions=array())
	{
		self::resolveNameID($model,$attribute,$xulOptions);
		$selection=self::resolveValue($model,$attribute);
		if($model->hasErrors($attribute))
	 ...
	 * @see radioButtonList
	 */
	public static function activeRadioButtonList($model,$attribute,$data,$xulOptions=array())
	{
		self::resolveNameID($model,$attribute,$xulOptions);
		$selection=self::resolveValue($model,$attribute);
		if($model->hasErrors($attribute))
	 ...
	 * @see errorSummaryCss
	 */
	public static function errorSummary($model,$header=null,$footer=null,$xulOptions=array())
	{
		$content='';
		if(!is_array($model))
			$model=array($model);
	...
	public static function error($model,$attribute,$xulOptions=array())
	{
		...
		if($error!='')
		{
			...
			return self::tag('div',$xulOptions,$error);
		}
		else
			return '';
	}
	...
	protected static function activeInputField($type,$model,$attribute,$xulOptions)
	{
		...
		if($type==='text' || $type==='password')
		{
			...
			else if($xulOptions['maxlength']===false)
				unset($xulOptions['maxlength']);
		}

		if($type==='file')
			unset($xulOptions['value']);
		else if(!isset($xulOptions['value']))
    ...
    protected static function activeTextbox($type,$model,$attribute,$xulOptions)
	{
		...
		if($model->hasErrors($attribute))
			self::addErrorCss($xulOptions);
		return self::tag('xul:textbox',$xulOptions);
	}
    /**
	...
	public static function listOptions($selection,$listData,&$xulOptions)
	{
		...
		{
			$content.='<option value="">'.strtr($xulOptions['prompt'],array('<'=>'&lt;', '>'=>'&gt;'))."</option>\n";
	...
	protected static function clientChange($event,&$xulOptions)
	{
		...
		if(isset($xulOptions['id']))
			...
			$id=$xulOptions['id']=isset($xulOptions['name'])?$xulOptions['name']:self::ID_PREFIX.self::$count++;
		...
		unset($xulOptions['params'],$xulOptions['submit'],$xulOptions['ajax'],$xulOptions['confirm'],$xulOptions['return'],$xulOptions['csrf']);
	}
    ...
    protected static function extendAttribute(&$xulOptions, $attribute, $value){
        ...
        if(strtolower(substr($attribute,0,2)) == 'on') $separator = ';';
        if($attribute == 'class') $separator = ' ';        
